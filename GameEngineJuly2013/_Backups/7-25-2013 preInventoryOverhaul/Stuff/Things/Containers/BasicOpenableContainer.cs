...
namespace Stuff.Things.Containers
{
	...
	public class BasicOpenableContainer : BasicContainer, OpenableContainer
	{
		...
		public BasicOpenableContainer(string name, string description, bool isOpaque,
			bool isSpecific = false, bool isPlural = false, bool isProper = false,
			bool canBeTaken = true, bool canBeDropped = true) :
			base(name, description, isSpecific, isPlural, isProper, canBeTaken, canBeDropped)
		{
			this.isOpen = false;
		...
		/// <summary>
		/// This override makes it so a closed, opaque container's contents are not considered visible.
		/// </summary>
		...
		public override string GetName()
		{
			if (this.isOpaque && !this.isOpen)
			{
				if (this.IsEmpty())
				{
					return base.GetName().Substring(base.GetName().IndexOf(' ') + 1);
				}
				else
				{
					return base.GetName().Substring(0, base.GetName().IndexOf('(') - 1);
				}
			}
			else
			{
